...
void ParticleExporter_Partio::writeParticlesPartio(const std::string& fileName, FluidModel* model)
{
	...
	// add time as part of attributes
	Partio::ParticleAttribute tAttr = particleData.addAttribute("time", Partio::FLOAT, 1);

	// add attributes that are defined in the .json file "particle attributes"
	...
	TimeManager* tm = TimeManager::getCurrent();	// in order to get current time data
	for (unsigned int i = 0; i < numParticles; i++)
	{
		...
		float* time = particleData.dataWrite<float>(tAttr, index);
