...
vector<float> SimulatorBase::TIME_STEPS;
...
void SimulatorBase::run()
{
	initSimulation();
	runSimulation();
	// record each timestep information
	// std::ofstream output_file("./TIME_STEPS.txt");
    // std::ostream_iterator<float> output_iterator(output_file, "\n");
    // std::copy(TIME_STEPS.begin(), TIME_STEPS.end(), output_iterator);
	cleanup();
}
...
void SimulatorBase::step()
{
	...
	*/
	// TIME_STEPS.push_back(TimeManager::getCurrent()->getTimeStepSize());
	// output frame data each simulation step (once update, output the update, rather than in a exportFPS manner)
	for (size_t i = 0; i < m_particleExporters.size(); i++)
	{
		m_particleExporters[i].m_exporter->step(m_frameCounter);
	}
